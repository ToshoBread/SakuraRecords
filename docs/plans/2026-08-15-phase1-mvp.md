...
export function usePOs() {
  ...
  function getQuarterStart(date: Date): Date {
    ...
    if (month >= 2 && month <= 4) return new Date(date.getFullYear(), 2, 1)       // Q2: Mar-May
    if (month >= 5 && month <= 7) return new Date(date.getFullYear(), 5, 1)       // Q3: Jun-Aug
    if (month >= 8 && month <= 10) return new Date(date.getFullYear(), 8, 1)      // Q4: Sep-Nov
    return new Date(date.getFullYear() - 1, 11, 1)                                // Q1: Dec-Feb (starts Dec of prev year)
  }
