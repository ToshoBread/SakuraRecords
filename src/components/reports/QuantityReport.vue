...
import type { QuantityByProductRow, MonthInfo } from '@/composables/useReports'
...
function exportCSV() {
  ...
  downloadCSV('quantity-by-product.csv', headers, rows)
}
</script>
...
<template>
  <Card>
    <CardHeader class="flex flex-row items-center justify-between">
      <CardTitle>Quantity Per Product Per Month</CardTitle>
          ...
          <Table>
            ...
            <TableBody>
              <TableRow v-for="row in data" :key="row.productCode">
                ...
                <TableCell v-for="m in months" :key="m.label" class="text-right">{{ row.months[m.label] ?? 0 }}</TableCell>
